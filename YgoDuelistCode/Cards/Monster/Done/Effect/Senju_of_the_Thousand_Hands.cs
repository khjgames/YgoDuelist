...
public sealed class Senju_of_the_Thousand_Hands : EffectMonsterCard
{
    public override int AttackPortionCount => 3;
    public Senju_of_the_Thousand_Hands()
        : base(
            cost: 1,
    ...
    public override YgoCardPackTags PackTags => YgoCardPackTags.MultiplayerSafe | YgoCardPackTags.Ritual | YgoCardPackTags.Light | YgoCardPackTags.Spell;
    ...
    protected internal override async Task OnSummoned(Player player, PlayerChoiceContext choiceContext, Creature duelMonsterPet) =>
        await RunOnNormalOrTributeSummonAsync(
            ...
            ctx => YgoRitualDeckSearchService.TrySearchAndAddToHandAsync(
                player,
                ctx,
                RitualDeckSearchKind.RitualMonsterOnly));
