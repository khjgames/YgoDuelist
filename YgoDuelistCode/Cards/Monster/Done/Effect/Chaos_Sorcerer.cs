...
/// <summary>
/// Hand: banish 1 LIGHT and 1 DARK from your Graveyard to Special Summon.
/// Activate (once per turn): banish 1 other monster you control; gain {Mgc} Chaotic Evolution.
/// </summary>
public sealed class Chaos_Sorcerer : EffectMonsterCard, IMonsterActivatedEffect
{
    private static readonly LocString BanishLightPrompt = new("cards", "YGODUELIST-CHAOS_SORCERER.banish_light_graveyard");
    private static readonly LocString BanishDarkPrompt = new("cards", "YGODUELIST-CHAOS_SORCERER.banish_dark_graveyard");
    private static readonly LocString BanishOtherControlledMonsterPrompt =
        new("cards", "YGODUELIST-CHAOS_SORCERER.banish_other_controlled_monster");

    public Chaos_Sorcerer()
        : base(
            cost: 1,
    ...
    public string ActivatedEffectDescriptionLocKey => "YGODUELIST-CHAOS_SORCERER.activated_effect.description";
    ...
    public bool IsActivatedEffectAvailable
    {
        get
        {
            ...
            if (pet == null || !MonsterCommandRegistry.TryGet(pet, out var cmd) || cmd.HasUsedActivatedEffectThisTurn)
                return false;
            return BuildOtherControlledFaceUpMonsterCandidates(Owner, excludeSource: this).Count > 0;
        }
    }

    public async Task OnActivatedEffect(PlayerChoiceContext choiceContext, CardPlay cardPlay, NormalMonsterCard source)
    {
        if (source is not Chaos_Sorcerer || Owner?.Creature?.CombatState == null)
            return;
        ...
        await DuelMonsterPetDeathPatch.ReleaseLiveFieldMonsterToBanishedAsync(player, chosen.Value.pet, chosen.Value.card);
