...
public sealed class Statue_of_the_Wicked : BaseTrapCard, IYgoAfterFaceDownSetTrapDestroyedToGraveyardAsync
{
    ...
    public override YgoCardPackTags PackTags => YgoCardPackTags.MultiplayerSafe | YgoCardPackTags.Dark | YgoCardPackTags.Trap | YgoCardPackTags.Earth;
    ...
    public async Task OnAfterFaceDownSetTrapDestroyedToGraveyardAsync(Player player)
    {
        ...
        if (!DuelMonsterSummon.HasRoomForDuelSummonAfterReleasing(player, 0))
            return;
        if (!ReactorSlimeSummonGate.AllowsSummonPrintedRace(player, DuelMonsterRace.Fiend))
            return;
