...
/// <summary>
/// <see cref="Shining_Angel"/>: when destroyed by battle and sent to the Graveyard, optional activation then
/// Special Summon 1 LIGHT monster with printed ATK 15 or less from the deck.
/// </summary>
public static class YgoShiningAngelGraveyard
{
    ...
    {
        if (addedCard is not Shining_Angel sa)
            return;
        if (!YgoShiningAngelBattleDeathGate.Consume(sa))
            return;
        ...
        TaskHelper.RunSafely(RunAsync(player, sa));
    }
    ...
    private static List<BaseMonsterCard> CollectDeckCandidates(Player player)
    {
        ...
        return draw.Cards
            ...
            .Where(m => m.DuelMonsterAttribute == DuelMonsterAttribute.Light && m.BaseAtk <= 15 && m.CanSummonDuelMonster)
            .ToList();
    }

    private static async Task RunAsync(Player player, Shining_Angel sourceInGraveyard)
    {
        ...
        if (activationPick.FirstOrDefault() is not Shining_Angel)
            return;
