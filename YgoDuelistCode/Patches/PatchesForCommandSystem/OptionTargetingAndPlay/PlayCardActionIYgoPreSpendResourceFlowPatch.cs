...
/// <summary>
/// Delegates pre-spend grids for spells that implement <see cref="IYgoPlayCardActionPreSpendResourceFlow"/>.
/// </summary>
[HarmonyPatch(typeof(PlayCardAction), "ExecuteAction")]
[HarmonyPriority(800)]
public static class PlayCardActionIYgoPreSpendResourceFlowPatch
{
    ...
    private static bool Prefix(PlayCardAction __instance, ref Task __result)
    {
        ...
        if (card is not IYgoPlayCardActionPreSpendResourceFlow flow)
            return true;
        ...
        __result = ExecuteWithPreSpendAsync(__instance, card, flow);
        return false;
    }

    private static async Task ExecuteWithPreSpendAsync(
        PlayCardAction action,
        CardModel card,
        IYgoPlayCardActionPreSpendResourceFlow flow)
    {
        try
        {
            if (!await flow.TryPreparePreSpendPlayAsync(action, action.Player, card))
            {
                action.Cancel();
                return;
            }
        ...
        {
            flow.ClearPreSpendPlayState(card);
        }
    }
